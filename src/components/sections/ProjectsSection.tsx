      ...
      <div className="container-width">
        <div className="space-y-8 lg:space-y-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8">
            {projects.map((project, index) => <div key={index} className="glass-card p-4 sm:p-6 hover-glow transition-all duration-300 space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {project.technologies.map((tech, idx) => <span key={idx} className="px-2 sm:px-3 py-1 bg-primary/10 text-primary rounded-full text-xs sm:text-sm font-medium">
                      {tech}
                    </span>)}
                </div>
